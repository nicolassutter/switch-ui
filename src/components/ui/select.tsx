...
export const selectFieldLabel = cva(
  "text-sm data-[disabled]:(cursor-not-allowed opacity-70) font-medium mb-1 inline-block",
  {
    variants: {
      label: {
...
type selectLabelProps<T extends ValidComponent = "span"> = ParentProps<
  SelectLabelProps<T> & { class?: string }
>;

export const SelectLabel = <T extends ValidComponent = "div">(
  ...
  return (
    <SelectPrimitive.Label
      class={cn(selectFieldLabel(), local.class)}
      {...rest}
    >
      {rest.children}
    </SelectPrimitive.Label>
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        class={cn(
          "relative z-50 min-w-[8rem] overflow-hidden rounded-md border dark:border-muted bg-popover text-popover-foreground shadow-md data-[expanded]:(animate-in fade-in-0 zoom-in-95) data-[closed]:(animate-out fade-out-0 zoom-out-95)",
          local.class
        )}
        {...rest}
      >
...
export const SelectItem = <T extends ValidComponent = "li">(
  ...
  return (
    <SelectPrimitive.Item
      class={cn(
        "relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 pl-2 pr-8 text-sm outline-none focus:(bg-accent text-accent-foreground) data-[disabled]:(pointer-events-none opacity-50)",
        local.class
      )}
      {...rest}
    >
